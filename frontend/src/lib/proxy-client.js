export async function dbExecute(payload) {
    try {
 ...
 */
export async function downloadFileFromDb(collection, docId, fileIndex, // mantido para retrocompatibilidade da URL da api, mas sempre 0
fileName, eventId) {
    const token = localStorage.getItem('auth_token');
    const baseURL = import.meta.env.VITE_API_URL || '';
 ...
 */
export async function fetchEntityEvents(streamType, streamId) {
    const token = localStorage.getItem('auth_token');
    const baseURL = import.meta.env.VITE_API_URL || '';
 ...
 */
export async function fetchEventStoreLogs(options) {
