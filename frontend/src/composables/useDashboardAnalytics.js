...
export function useDashboardAnalytics(patients, indicators, startDate, endDate) {
    return computed(() => {
        ...
        let eList = pList.flatMap(p => (p.events || []).map((e) => ({ ...e, patientId: p._id, patientName: p.name })));
        if (start || end) {
        ...
        const recentEvents = eList.filter((e) => new Date(e.occurrenceDate) >= thirtyDaysAgo);
        const previousEvents = eList.filter((e) => {
            const d = new Date(e.occurrenceDate);
        ...
        const indicatorsCards = iList.map(ind => {
            ...
            if (isAdId) {
                ...
                const modalityCounts = { 'AD': 0, 'ID': 0 };
                pList.forEach(p => {
                    ...
                    if (adIdEvents.length > 0) {
                        ...
                        if (subName.includes('AD'))
                            modalityCounts['AD']++;
                        else if (subName.includes('ID'))
                            modalityCounts['ID']++;
                    }
                });
        ...
        const adverseEvents = eList.filter((e) => e.indicator?.name.toLowerCase().includes('adverso') && e.subindicator);
        const subCountsAdv = {};
        ...
        const ouvidoriasEvents = eList.filter((e) => e.indicator?.name.toLowerCase().includes('ouvidoria') && e.subindicator);
        const subCountsOuv = {};
